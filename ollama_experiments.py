...
# now = time.time()
# stream = ollama.chat(model='gemma:2b', stream=False, messages=[
#   {
#     # 'role': 'system', 'content': 'You are given a job listing. You only repsond in valid JSON format. Do not response with any other format.',
#     'role': 'user',
#     'content': '''What is the industry of this job listing: Multiplatform Editor, Newsletters (Temporary)
...
for i in range(0, 5):
  now = time.time()
  response = ollama.generate(model='gemma:2b', prompt='''Can you give me the industry (string), responsibilities (array of strings), and qualifications (array of strings) of this job listing. Find as many responsibilities and qualifications as it says: Multiplatform Editor, Newsletters (Temporary)
  The Los Angeles Times, 2300 E. Imperial Hwy, El Segundo, California, United States of America Req #20
  Saturday, June 22, 2024
  ...
  Other details
  Job Family Los Angeles Times Super Co. Job Function Individual Contributor Pay Type Hourly Min Hiring Rate $39.59 Max Hiring Rate $45.77
  Apply Now''', options={'temperature': 0.2})
  now = time.time() - now
  print(response['response'])
  parsed = parse_output(response['response'])
  print(parsed)
  print(now)