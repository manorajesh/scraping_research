...
stream = ollama.chat(model='gemma:2b', stream=False, messages=[
  {
    # 'role': 'system', 'content': 'You input a job listing. You will now extract the relevant data from the job listing exactly as it appears in the listing, getting as many entries for each key as possible. You always respond in valid JSON.',
    'role': 'user',
    'content': '''Can you give me the job title (string), industry (string), responsibilities (array of strings), qualifications (array of strings) of this job listing with valid JSON. Find as many responsibilities and qualifications as it says: Multiplatform Editor, Newsletters (Temporary)
